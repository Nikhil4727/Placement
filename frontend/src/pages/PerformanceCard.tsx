...
const EnhancedFileUploader: React.FC = () => {
  ...
  // Handle file upload
 // In the EnhancedFileUploader component
// Add a new state for replacement mode
...
const handleUpload = async () => {
  ...
  uploadData.append('replace', isReplacement.toString()); // Add replacement flag
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white flex items-center">
          <FileText className="h-20 w-8 mr-2 text-blue-600" />
          Training Dashboard
        </h1>
      ...
      <div className="bg-white shadow-lg rounded-xl overflow-hidden">
        {activeView === 'upload' ? (
          <div className="p-6">
            ...
            {/* Add this before the upload button in the upload form */}
<div className="mt-4 flex items-center">
  ...
  />
  <label htmlFor="replace-checkbox" className="ml-2 block text-sm text-gray-700">
    Replace existing file for this year and course
  </label>
</div>
...
{isReplacement && (
  <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
    <p className="text-sm text-yellow-700">
      ...
      This will replace any existing file with the same year and course. Student data will be updated.
    </p>
  </div>
)}
...
//     try {
//       const res = await axios.get('http://localhost:5000/data');
//       console.log('Dropdown data:', res.data);
//       setCourses(res.data.courses);
//       setYears(res.data.years);
//     } catch(err) {
...
//       let queryString = '';
//       if (filters.year !== 'All') queryString += `year=${filters.year}&`;
//       if (filters.course !== 'All') queryString += `course=${filters.course}`;
      
//       const url = `http://localhost:5000/api/files${queryString ? '?' + queryString : ''}`;
...
//   // Filter years and courses for dropdowns
//   const yearOptions = ["All", ...(years.map(y => y.year))];
//   const courseOptions = ["All", ...(courses.map(c => c.coursename))];
...
//             >
//               <Upload className="w-5 h-5 mr-2" />
//               {isUploading ? 'Uploading...' : isReplacement ? 'Replace File' : 'Upload File'}
//             </button>
//           </div>
//         ) : (
//           <div className="p-6">
